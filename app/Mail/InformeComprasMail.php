...
class InformeComprasMail extends Mailable
{
    ...
    {
        // Generar el PDF
        $pdf = Pdf::loadView('informes.mis-compras', $this->pdfData)
            ->setPaper('letter', 'portrait');

        $fecha_hora = Carbon::now()->format('Y_m_d_H_i_s');
        $nombreArchivo = "mis_compras_{$fecha_hora}.pdf";

        return $this->subject('Tu Informe de Compras - VASIR')
                    ->view('emails.informe-compras')
                    ->with([
