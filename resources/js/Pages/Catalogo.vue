...
Gracias.`
    ...
    if (isMobile) {
      ...
      const mailtoUrl = `mailto:${config.value.admin_email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
      window.location.href = mailtoUrl
    } else {
      // En escritorio, abrir Gmail web
      const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${config.value.admin_email}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
