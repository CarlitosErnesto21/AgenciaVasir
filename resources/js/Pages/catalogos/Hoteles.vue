...
const filteredHoteles = computed(() => {
    ...
    if (filters.value.global.value) {
        ...
        filtered = filtered.filter(hotel =>
            ...
            (hotel.categoria_hotel?.nombre && hotel.categoria_hotel.nombre.toLowerCase().includes(searchTerm)) ||
...
const fetchCategoriasHoteles = async () => {
    try {
        ...
        categoriasHoteles.value = response.data.data || response.data || [];
    } catch (err) {
        categoriasHoteles.value = [];
        toast.add({
            severity: "error",
            summary: "Error",
    ...
    <AuthenticatedLayout>
        ...
        <div class="container mx-auto px-4 py-6">
            ...
            >
                ...
                <Column field="categoria_hotel.nombre" header="Categoría" class="w-32 hidden md:table-cell">
                    <template #body="slotProps">
                        <div class="text-sm leading-relaxed">
                            {{ slotProps.data.categoria_hotel?.nombre || 'Sin categoría' }}
                        </div>
                    </template>
                </Column>
                ...
                <Column field="estado" header="Estado" class="w-28 hidden sm:table-cell">
                    <template #body="slotProps">
                        <span :class="'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ' + (slotProps.data.estado === 'activo' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800')">
            ...
            <Dialog v-model:visible="dialog" :header="btnTitle + ' Hotel'" :modal="true" :style="dialogStyle" :closable="false" :draggable="false">
                <div class="space-y-4">
                    ...
                    <div class="w-full flex flex-col">
                        <div class="flex items-center gap-4 sm:gap-5">
                            <label for="categoria" class="w-36 sm:w-32 flex items-center gap-1">
                            ...
                            <Select
                                ...
                                class="w-full rounded-md border-2 border-gray-400 hover:border-gray-500"
                                placeholder="Seleccionar"
                                :class="{'p-invalid': submitted && !hotel.categoria}"
                            />
                        </div>
                        <small class="text-red-500 ml-28" v-if="submitted && !hotel.categoria">
